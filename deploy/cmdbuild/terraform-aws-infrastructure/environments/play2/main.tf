...
locals {
  ...
  troubleshooter = "176.46.126.158/32,81.20.149.200/32" #pol-home, CG-office
}

# Get DB credentials and parse JSON
data "aws_secretsmanager_secret_version" "play2_creds" {
  secret_id = "${local.env}-db-admin"
}
locals {
  db_creds = jsondecode(
    data.aws_secretsmanager_secret_version.play2_creds.secret_string
  )
}

# Add SSH keys
resource "aws_key_pair" "play2_key" {
...
resource "aws_key_pair" "play2_user" {
...
# Allocate Elastic IP
resource "aws_eip" "play2_eip" {
  vpc      = true
  instance = module.play2_ec2.id
...
module "play2_vpc" {
  ...
  name = "cg-infra-security-4"
  cidr = "172.16.4.0/24"

  azs            = ["eu-central-1a", "eu-central-1b"]
  public_subnets = ["172.16.4.0/25", "172.16.4.128/25"]
...
module "play2_ec2" {
  ...
  key_name                = aws_key_pair.play2_user.key_name
  ...
  subnet_id                   = module.play2_vpc.public_subnets[0]
  associate_public_ip_address = true

  vpc_security_group_ids = [module.play2_security_group.security_group_id]
...
module "play2_security_group" {
  source = "terraform-aws-modules/security-group/aws"
  version = "~> 4.0"

  name        = "${local.env}-app-secgr"
  description = "${local.env}-app-secgr rules"
  vpc_id      = module.play2_vpc.vpc_id

  ingress_cidr_blocks = [module.play2_vpc.vpc_cidr_block]
  ingress_with_cidr_blocks = [
    ...
    },
    {
      ...
      to_port     = 8080
      protocol    = "tcp"
      description = "Ingress HTTP from specific source"
      cidr_blocks = local.troubleshooter
    }
  ]

  egress_cidr_blocks = [module.play2_vpc.vpc_cidr_block]
...
module "play2_security_group_db" {
  source = "terraform-aws-modules/security-group/aws"
  version = "~> 4.0"
...
module "play2_db" {
  ...
  subnet_ids = module.play2_vpc.public_subnets
  ...
  vpc_security_group_ids = [module.play2_security_group_db.security_group_id]
